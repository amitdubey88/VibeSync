    ...
    <AnimatePresence>
      ...
      >
        <Motion.div
          ...
          className="bg-[#0a0a0b]/95 border border-white/5 p-6  w-full max-w-sm shadow-[0_10px_50px_rgba(0,0,0,0.8)] relative"
        >
          <button 
            onClick={onClose}
            className="absolute top-4 right-4 p-1 text-zinc-500 hover:text-zinc-100  hover:bg-white/5 transition-colors"
          >
          ...
          <div className="space-y-4">
            {shortcuts.map((s, i) => (
              <div key={i} className="flex justify-between items-center">
                ...
                <kbd className="px-3 py-1.5 bg-white/5 border border-white/10  text-xs font-mono text-fuchsia-400 shadow-sm font-bold tracking-widest">
                  {s.key}
                </kbd>
              </div>
            ))}
