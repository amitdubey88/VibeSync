    ...
    <div className="flex flex-col h-full bg-black/40 backdrop-blur-xl border-l border-white/5 p-4">
      <div className="mb-4">
        <h3 className="text-white font-semibold text-lg flex items-center gap-2">
          ...
          <span className="px-2 py-0.5 rounded-full bg-fuchsia-500/20 text-fuchsia-400 text-[10px] uppercase font-bold tracking-wider">Host</span>
        </h3>
      ...
      <form onSubmit={handleAddWord} className="flex gap-2 mb-4">
        <input
          ...
          className="flex-1 bg-black border border-white/10 rounded-lg px-3 py-2 text-sm text-zinc-200 focus:outline-none focus:border-fuchsia-500 font-headline tracking-wide transition-colors"
        />
        <button
          type="submit"
          disabled={!newWord.trim()}
          className="bg-fuchsia-600 hover:bg-fuchsia-500 shadow-[0_0_10px_rgba(217,70,239,0.3)] disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors"
        >
          Block
        </button>
      </form>
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-2">
        {bannedWords.length === 0 ? (
          <div className="text-center text-zinc-500 text-sm mt-8 border border-dashed border-white/10 font-headline rounded-xl p-6">
          ...
          <div className="flex flex-wrap gap-2">
            {bannedWords.map(word => (
              <Motion.span
                ...
                key={word}
                className="inline-flex items-center gap-2 px-3 py-1.5 bg-fuchsia-500/10 border border-fuchsia-500/30 text-fuchsia-200 font-headline tracking-wide rounded-full text-sm"
              >
                {word}
                <button
