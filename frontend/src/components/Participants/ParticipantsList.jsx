...
import { Crown, Wifi, WifiOff, Loader2, Mic, MicOff, MoreVertical, UserCheck, VolumeX, LogOut } from 'lucide-react';
            ...
            <div key={p.userId} className="participant-row relative">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  {isRoomHost && <Crown className="w-3.5 h-3.5 text-accent-yellow shrink-0" title="Host" />}
              ...
              {canControl && (
                <div className="relative shrink-0" ref={openMenuId === p.userId ? menuRef : null}>
                  ...
                  {openMenuId === p.userId && (
                    <div className="absolute right-0 top-7 z-50 w-44 rounded-lg bg-bg-card border border-border-dark shadow-xl overflow-hidden">
                      <button
                        onClick={() => { transferHost(p.userId); setOpenMenuId(null); }}
                      ...
                      >
                        <UserCheck className="w-4 h-4 text-accent-yellow" />
                        Make Host
                      </button>
                      <button
                        onClick={() => { muteParticipant(p.userId); setOpenMenuId(null); }}
                      ...
                      >
                        <VolumeX className="w-4 h-4 text-accent-purple" />
                        Mute
                      </button>
                      <div className="border-t border-border-dark" />
                      <button
                        onClick={() => { kickParticipant(p.userId); setOpenMenuId(null); }}
